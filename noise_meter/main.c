...
int main (int argc, char *argv[]) {// ./main default STATION_x EVENT_NAME
	...
	float coef_b[] = {0.169994948147430,0.280415310498794,-1.120574766348363,0.131562559965936,0.974153561246036,-0.282740857326553,-0.152810756202003};
	float coef_a[] = {1.00000000000000000,-2.12979364760736134,0.42996125885751674,1.62132698199721426,-0.96669962900852902,0.00121015844426781,0.04400300696788968};
...
//-------------------------------CALIBRAÇÃO-------------------------------------------------	
	...
	while(1){
		...
		if((calibrator_buff->count + BLK_SIZE) > (WaveRate * SEGMENT_DURATION)){//check if buffer can receive the new BLK_SIZE block
		...
		snd_pcm_sframes_t read_frames = snd_pcm_readi(capture_handle,calibrator_buff->buff_16,step);//leitura de amostras
		...
		if(seconds >= (CALIBRATION_TIME - 5)){//ultimos 5 segundos para calibração
			...
			bufferInternalCopy(calibrator_buff, step);//COPIAR DO ARRAY DE 16 BITS PARA O DE 32
...
//-----------------------------------------------------------	
	...
	while (1) { 
		...
		snd_pcm_sframes_t read_frames = snd_pcm_readi(capture_handle, buff_1->buff_16, window_step);//leitura de amostras da placa de som
